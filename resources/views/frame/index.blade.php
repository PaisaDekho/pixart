                        ...
                        <div class="card-body position-relative z-index-1 p-3">
                           <div style="width: 100%;height: 56px;position: absolute;left: 0;border-radius: 16px 16px 0 0px;top: 0;background-color: rgb(12 12 12 / 17%);z-index: -1;"></div>
                            ...
                            <div class="form-check form-switch">
                              <input class="form-check-input featured-switch" type="checkbox" data-id="{{$frame->id}}" @if($frame->featured==1) checked @endif>
                              <label class="form-check-label" for="rememberMe">Featured</label>
                            </div>
                            
                            <label style="background:green;border-radius:20px;" class="text-white text-sm p-1 mt-7" >{{$frame->ratio}}</label>
                            
                            <h5 class="text-black text-sm mt-0 pb-0 " >{{$frame->title}}</h5>
                            <div class="" >
...
<script>
    ...
    $(".featured-switch").change(function(){
        var checked = $(this).is(':checked');
        var id = $(this).data("id");
        
        $.ajax({
          type: "POST",
          url: "{{ secure_url('/frame-featured') }}",
          data: { checked : checked , id : id},
          headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
          success: function(data) {
              
              if(checked){
                  toastr.success("Frame Featured");
              }else{
                  toastr.success("Frame Unfeatured");
              }
          },
          error (data) {
              toastr.error(JSON.stringify(data));
